...
def single_byte_xor(value, s):
    return bytes([value ^ byte for byte in s])
...
eng_letter_bytes_frequencies = {ord(k): v for k, v in eng_letter_frequencies.items()}
...
def get_english_score(s):
    ...
    for letter in s_frequencies:
        score += abs(s_frequencies[letter] / len(s) - eng_letter_bytes_frequencies.get(letter, 0))
    return score
